...
public class BluetoothConnectivity extends ManualTestsProgressBarActivity {
    ...
    private Handler handler = new Handler()
    {
        ...
        public void handleMessage(Message msg) {
            ...
            if(TextUtils.isEmpty(message)){
                Toast.makeText(BluetoothConnectivity.this, getResources().getString(R.string.bluetooth_toast), Toast.LENGTH_SHORT).show();
            }
            manualTestResultDialog(mTestName, result, BluetoothConnectivity.this);
        }
    };
        ...
        if(AppUtils.VersionUtils.hasQ())
        {
            if (DeviceInfo.getInstance(this).isGPSEnabled()) {
                if (NetworkUtil.isOnline()) {
                    ManualTest.getInstance(BluetoothConnectivity.this).performTest(TestName.BLUETOOTHCONNECTIVITYTEST, handler);
                }
            } else {
        ...
        switch (requestCode){
            case StartLocationAlert.RC_GPS_SETTINGS:
                if(resultCode!= Activity.RESULT_OK)
                    ...
                    manualTestResultDialog(mTestName, TestResult.FAIL, BluetoothConnectivity.this);
                break;
        }
